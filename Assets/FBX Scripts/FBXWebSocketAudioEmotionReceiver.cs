...
public class FBXWebSocketAudioEmotionReceiver : MonoBehaviour
{
    ...
    public FBXEmotionController emotionController;
    ...
    private IEnumerator PlayEmotionSequence(EmotionPayload[] lines, float totalDuration, string subtitle)
    {
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            {
                FBXBlendShapePreset preset = ConvertEmotion(line.emotion);
                emotionController.TriggerEmotion(preset, clampedDuration, line.confidence);
            }
        }

        if (emotionController != null && emotionController.blendShapeProxy != null)
        {
            emotionController.blendShapeProxy.ImmediatelySetValue(FBXBlendShapePreset.Neutral, 1.0f);
        }
    }

    private FBXBlendShapePreset ConvertEmotion(string emotion)
    {
        return emotion.ToLower() switch
        {
            "joy" => FBXBlendShapePreset.Joy,
            "angry" => FBXBlendShapePreset.Angry,
            "sad" => FBXBlendShapePreset.Sad,
            "neutral" => FBXBlendShapePreset.Neutral,
            "blink" => FBXBlendShapePreset.Blink,
            _ => FBXBlendShapePreset.Neutral,
        };
    }
